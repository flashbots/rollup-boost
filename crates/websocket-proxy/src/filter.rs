...
impl FilterType {
    ...
    pub fn new_combined_with_mode(addresses: Vec<String>, topics: Vec<String>, match_mode: MatchMode) -> Self {
            ...
            for receipt_value in receipts.values() {
                if let Some(receipt_obj) = receipt_value.as_object() {
                    for receipt_data in receipt_obj.values() {
                        if let Some(logs) = receipt_data
                            .get("logs")
                            .and_then(|logs| logs.as_array())
                        {
                            for log in logs {
                                if let Some(addr_str) = log
                                    .get("address")
