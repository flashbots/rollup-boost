...
| Leader `rollup-boost` Unhealthy | Rollup Boost Failure | Leader sequencer `rollup-boost` becomes unhealthy, causing `op-conductor`s sequencer health checks to fail. If `op-conductor` is configured to failover in the event that `rollup-boost` is partially healthy conductor will elect a new leader. This failure mode is the same as a typical leader sequencer failure. Once the sequencer recovers, it will continue to participate in the cluster as a follower. If `op-conductor` is configured not to failover in the event of `rollup-boost` partial health, the sequencer will continue to publish the default exectuion client's blocks until the builder has recovered back to full health. |
...
| Leader Builder Stops Producing Blocks | Builder Failure | The builder associated with the sequencer leader stops producing new payloads. `rollup-boost` will detect the builder failure via background health checks and downgrade its health status to partial. This will result in `rollup-boost` ignoring the builder and selecting the default execution client's payload for block production. `op-conductor` will notice the leader's degraded health and (if configured to failover upon parital health), attempt to elect a fully healthy sequencer (if one is available). Once the builder recovers and resumes payload production, `rollup-boost` will update its health to `200` and normal operation will resume. |
| Leader Builder Falls Out of Sync | Builder Failure | The builder associated with the sequencer leader falls out of sync with the chain head. `rollup-boost` will detect the unsynced state via the background health checks and downgrade its health status to partial. This will result in `rollup-boost` ignoring builder payloads and selecting the default execution client payload for block until the builder is resynced. `op-conductor` will notice that the leader sequencer health has degraded and (if configured to failover upon parital health) attempt to elect a new leader if a fully healthy sequencer if available. Once the builder recovers, `rollup-boost` will update its health to `200` and normal operation will resume. |
