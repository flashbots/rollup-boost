...
| Follower `rollup-boost` Unhealthy | Rollup Boost Failure | Follower sequencer `rollup-boost` becomes unhealthy. The leader sequencer is unaffected. Once `rollup-boost` recovers, the sequencer will rejoin the set as a follower. |
