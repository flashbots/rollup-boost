...
where
    C: EngineApiClient,
{
    pub fn new(l2_client: Arc<C>, builder_client: Arc<C>, boost_sync: bool) -> Self {
        Self {
            l2_client,
            builder_client,
            boost_sync,
...
mod tests {
    ...
    pub struct MockEngineServer {
        fcu_requests: Arc<Mutex<Vec<(ForkchoiceState, Option<OptimismPayloadAttributes>)>>>,
        ...
        get_payload_response: RpcResult<OptimismExecutionPayloadEnvelopeV3>,
    ...
    impl MockEngineServer {
        pub fn new() -> Self {
            Self {
                ...
                get_payload_response: Ok(OptimismExecutionPayloadEnvelopeV3{
    ...
    impl TestHarness {
        async fn new(
            ...
            let eth_engine_api =
                EthEngineApi::new(Arc::new(l2_client), Arc::new(builder_client), boost_sync);
            let mut module: RpcModule<()> = RpcModule::new(());
            module.merge(eth_engine_api.into_rpc()).unwrap();
    ...
    async fn engine_success() {
        ...
        let req: &(ForkchoiceState, Option<OptimismPayloadAttributes>) =
            fcu_requests_mu.get(0).unwrap();
        assert_eq!(req.0, fcu);
    ...
    async fn spawn_server(mock_engine_server: MockEngineServer, addr: &str) -> ServerHandle {
        ...
        module
            .register_method("engine_forkchoiceUpdatedV3", move |params, _, _| {
                let params: (ForkchoiceState, Option<OptimismPayloadAttributes>) =
                    params.parse()?;
