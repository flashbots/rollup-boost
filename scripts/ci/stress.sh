...
run() {
    ...
    docker run --rm --network host -v /tmp/.contender:/root/.contender contender spam --tps 50 --min-balance 0.1 -p $PREFUNDED_PRIV_KEY -r $OP_RETH_BUILDER_SOCKET --optimism fill-block
}
